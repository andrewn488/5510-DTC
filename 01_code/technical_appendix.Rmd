...
trafficReportList %>% 
  select(`Property Name`, num_status) %>%
  tabyl(`Property Name`, num_status)
```
...
costPerCounty <- GLCountyGrouping %>%
  ...
  mutate(costPerLead = DifferenceDebitCredit/countLeadCounty,
         costForAllResidents = costPerLead * ResidentStatusCount,
         unallocatedBudget = DifferenceDebitCredit - costForAllResidents) %>%
  rename(TotalSpending = DifferenceDebitCredit, countLead = countLeadCounty) %>%
  select(County, TotalSpending, countLead, ResidentStatusCount, ResidentRate,  costPerLead, costForAllResidents, unallocatedBudget)
