...
trafficReportList <- trafficReportList %>% 
  ...
  mutate("Event Date" = as.Date("Event Date", format = "%mm/%dd/%YYYY"))
...
# recode Result vector
trafficReportList <- trafficReportList %>% 
  mutate(num_result = case_when(trafficReportList$Status == 'Resident' ~ 1,
                                trafficReportList$Status == 'Approved' ~ 1,
                                trafficReportList$Status == 'Prospect' ~ 0, 
                                trafficReportList$Status == 'Denied' ~ 0,
                                trafficReportList$Status == 'CanceledGuest' ~ 0,
...
```{r filter Source and Result}
# filter 'Source' and 'Result' to top hits
trafficReportList_filter <- trafficReportList %>% 
...
```{r join Property List with Traffic Report}
...
# Source 
ggplot(data = trafficReportList_filter, mapping = aes(x = Source)) +
  geom_bar() + 
  coord_flip(), 

# Status
ggplot(data = trafficReportList_filter, mapping = aes(x = Status)) +
  geom_bar() + 
  coord_flip(),

# Result/ Reason
ggplot(data = trafficReportList_filter, mapping = aes(x = Result)) + 
  geom_bar() + 
  coord_flip(),
...
reg_spend <- lm(formula = , data = ledger_base)
...
+ 7/30/2022 Interpretation: 

    + Promote Roomate (0.62) is most positively significant
    + Transfer Unit (0.51) 2nd most positive
    + Majority are negatively associated - is this model correct?
    + Perhaps do a filter on Source and throw in an interaction 
    
```{r}
# normalize trafficReportList

trafficReportList_norm <- trafficReportList %>% 
  select(num_status)

trafficReportList_norm <- scale(trafficReportList_norm)
```
