...
trafficReportList <- trafficReportList %>% 
  recode(Source, "A-Frame", "A - Frame")


    recode(Source, "A-Frame = A - Frame")


trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Apartment list","Apartment List")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Applicant grouping","Applicant Grouping")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Desired floorplan not avail","Desired Floorplan not avail")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Desired floorplan not availabl","Desired Floorplan not avail")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Desired Floorplan not availabl","Desired Floorplan not avail")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Desired mone-in date not avail","Desired Move-in date not avail")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Desired move-in date not avail","Desired Move-in date not avail")
...
trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Desired move in date unavail","Desired Move-in date not avail")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Desired Move in date unavail","Desired Move-in date not avail")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Found another apartment","Found another Apartment")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Found another apartment (Unqualified)","Found another Apartment")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Future / 30-days out","Future / 30-Days Out")
...
trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Leased an Apartment","Leased")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Not moving","Not Moving")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Not moving (Unqualified)","Not Moving")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Personal (Unqualified)","Personal")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Promote Roommate","Roommate")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Screening process","Screening Process")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Screening process (Unqualified)","Screening Process")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Screening Process (Unqualified)","Screening Process")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Still looking / Undecided","Still Looking / Undecided")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Still looking / Undecided (Unqualified)","Still Looking / Undecided")
...
trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="StillLooking/ Undecided","Still Looking / Undecided")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Timeframe change","Timeframe Change")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Timeframe Change (Unqualified)","Timeframe Change")

trafficReportList$Source <-
replace(trafficReportList$Source,trafficReportList$Source=="Unqualified (Unqualified)","Unqualified")
```
...
trafficReportList <- trafficReportList %>% 
  ...
  mutate("Result/ Reason" = as.factor(`Result/ Reason`)) %>% 
  rename("Result/Reason" = "Result/ Reason") %>% 
...
# filter 'Source' to top 10
trafficReportList <- trafficReportList %>% 
  filter(Source %in% c("Apartmentlist.com", "Craigslist", 'Driveby', 
                       'Facebook', 'Google Plus', 'Property Website', 
                       'Zillow', 'Tarragon Website', 'Referrals', 'Rent.com')) %>% 
  filter(`Result/Reason` %in% c("Apartment List","Desired move-in date not avail", 
                                "Duplicate", "Found another apartment",
                                "Leased an Apartment", "Nothing", "Online Lead",
                                "Personal", "Still looking / Undecided", "Took Application"))
...
# Result/ Reason
ggplot(data = trafficReportList, mapping = aes(x = `Result/Reason`)) + 
  geom_bar() + 
  coord_flip(),
...
trafficReportList %>% 
  tabyl(`Result/Reason`) %>% # creates table of counts
  adorn_totals(where = c("row", "col")) %>%  # Total margins
  adorn_percentages(denominator = "all") %>%   # creates proportions
  adorn_rounding(2) 
